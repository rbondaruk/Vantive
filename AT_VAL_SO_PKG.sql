...
SELECT swRcvProdReleaseId,SUM(swOrigQuantity) AS SOLsum
FROM SW_SERVICE_ORD_LN
WHERE swServiceOrderId = SOId
AND swType = cs_MATERIAL_REQUEST
GROUP BY swRcvProdReleaseId
ORDER BY swRcvProdReleaseId;

cs_MATERIAL_REQUEST  SW_VALID_CODE.swValue%TYPE;
cs_AT_COMPLETED      SW_VALID_CODE.swValue%TYPE;
SOL_PR_id            SW_SERVICE_ORD_LN.swRcvProdReleaseId%TYPE;
SOL_PR_sum           SW_SERVICE_ORD_LN.swOrigQuantity%Type;
PU_PR_sum            SW_SERVICE_ORD_LN.swOrigQuantity%Type;
b_in                 int;
b_out                int;
cs_rc                int;
...
BEGIN
   ...
   LOOP
      BEGIN
         SOL_PR_id := c1_rec.swRcvProdReleaseId;
         SOL_PR_sum := c1_rec.SOLsum;

         SELECT SUM(swAddQty)
         INTO PU_PR_sum
         FROM SW_PARTS_USED
         WHERE swServiceOrderId = SOId
         AND swAddProdRelId = SOL_PR_id
         GROUP BY swAddProdRelId;

         IF (SOL_PR_sum <> PU_PR_sum) THEN
            BEGIN
			   status := 8178;
               RETURN;
            END;
